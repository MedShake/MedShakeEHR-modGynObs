...
class msModGynobsDataCourrier
{
    ...
    public static function getCourrierDataCompleteModule(&$d)
    {
        ...
        $marqueurs = $name2typeID->getTypeIDsFromName(['groFermetureSuivi', 'nouvelleGrossesse']);
        $marqueurs['patientID'] = $d['patientID'];
    ...
    public static function getCrDataCompleteModuleForm_gynObsMarqueursSeriques(&$d)
    {
        ...
        $name2typeID = new msData();
        $marqueurs = $name2typeID->getTypeIDsFromName(['echo12']);
        $marqueurs['patientID'] = $d['patientID'];
        $marqueurs['instance'] = $d['instance'];

        // echo 12 id + date
        if ($e12 = msSQL::sqlUnique("SELECT id, creationDate from objets_data where toID = :patientID and instance= :instance and typeID = :echo12 and deleted='' and outdated='' order by id desc limit 1", $marqueurs)) {
